    ...
    private stockholdService: StockholdService,private theme: NbThemeService) {
      this.themeSubscription = this.theme.getJsTheme().subscribe(config => {
        ...
        this.stockholdhiss.subscribe((stockData: StockHoldHis[]) => {
          ...
          const colors1: any[] = [colors.success, colors.primary,colors.dangerLight,colors.info,colors.warning];
          
    
          this.data = {
            labels: stockData[0].dates.map(date => this.formatDate(date)),// Assuming dates are the same for all stocks
            datasets: stockData.map((stock ,index)=> ({
              label: stock.name,
              data: stock.prices,
              borderColor: colors1[index%colors1.length] , // Replace 'default_color' with your desired color
